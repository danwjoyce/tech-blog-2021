...
**Note:** Before posting this, I read around social media and had a look at the literature on MDTs in clinical services particularly mental healthcare.  There are many detailed and thorough criticisms of MDTs including the ethics of making decisions about people's care without their consent or without involving them -- to be clear, this post does not address these issues in the provision of healthcare in any specific field or specialty.  It is more basic and reflects my trying to find a more formal understanding of the commonly-expressed view that having a team making decisions results in better decisions.  
...
Not all members of a team will have the same competencies; above, we used uniform competence (i.e. all members of the team voted correctly with the same probability).  In the machine learning literature -- where an MDT or jury is called an ensemble, mixture of experts or a multi-agent system -- ``diversity'' refers to the agreement (similarity) between different team members' output [@kuncheva2003measures] and there is an argument that the more diverse the team is, the better the overall performance of the team (however, as Kuncheva and Whittaker show, diversity is hard to formalise).

So we can ask the same question by simulating teams of different sizes ($N$) as well as each team member having different voting probabilities (competencies) $p_i$.  The detail of how we generate teams with a full spectrum of mixed-competencies is explained in the [Appendix](#appendix-b).

Assume we have, for a given team size $N$, the competencies of the team given by a vector $\mathbf{p} = (p_1, p_2, \ldots, p_N )$.
...
Of course, the diversity score is only really meaningful if you know the value of the highest performing team member, $p_{max}$.
...
plotResult <- function( df, N, anot = NULL ) {
  ...
  # p <- ggplot( df, aes( x = mean, y = majority.prob, colour = factor(p.max), group = factor(p.max) ) ) +
...
```
## Small Team Example
To begin with, we simulate a team of size $N=3$ (recall, we use odd $N$ to prevent tied votes).  Here's a list of the possible team compositions in terms of probabilities of voting correctly (competencies):

```{r, eval = TRUE, echo = FALSE}
ex.comp <- systematicDiversityParams(3, p.vals)
ex.comp <- ex.comp[ , c("V1","V2","V3","p.min","mean") ]
colnames(ex.comp) <- c("p1","p2","p3","p.min","mean")
rownames(ex.comp) <- NULL
ex.comp$name <- rep("",nrow(ex.comp))
ex.comp$name[c(1,9,10,16)] <- c("A","B","C","D")
kbl(ex.comp) %>% 
  kable_paper(full_width = TRUE) %>%
  row_spec(c(1:9), background  = "#66c2a5") %>%
  row_spec(c(10:14), background  = "#fc8d62") %>%
  row_spec(c(15:16), background  = "#8da0cb")
```
The three columns (`p1`,`p2` and `p3`) are the voting probabilities of each team member.  The rows are coloured by `p.min` or, the competence (probability of voting correctly) of the *least* competent member of the team (we'll use this shortly when we graph the performance of teams).  We have labelled four team compositions for special attention (A,B,C and D):
...
```{r, eval = TRUE, echo = FALSE}
ggplot( new.df, aes( y = diversity, x = majority.prob.bin, group = factor( p.max ), colour = factor( p.max ) ) ) +
  geom_point() +
  facet_wrap( vars(N) )
```
